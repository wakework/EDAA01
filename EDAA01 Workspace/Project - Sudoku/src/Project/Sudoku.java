...
public class Sudoku implements SudokuSolver {
	...
	// Rule check of known User-digits
	private boolean checkUser() {
		for (int i = 0; i < 9; i++) {
			for (int k = 0; k < 9; k++) {
				if (mainBoard[i][k] != 0) {
					if (!trySetNumber(i, k, mainBoard[i][k])) {
						return false;
					}
				}
			}
	...
	private boolean solve(int row, int col) { 
		// (Base), check User-chosen digits
		if (!checkUser()) {
			return false;
		}
		
		// Do not change User-chosen digits
		for (int i = 0; i < 9; i++) {
			for (int k = 0; k < 9; k++) {
				if (mainBoard[i][k] != 0) {
					if (!trySetNumber(i, k, getNumber(i, k))) {
						return false;
					}
					continue; 
				}

				// Check rules for each possible value
				for (int num = 1; num < 10; num++) {
					if (trySetNumber(i, k, num)) { 
						setNumber(i, k, num);
						if (solve(row, col)) { 
							return true;
						} else {
